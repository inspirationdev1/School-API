...
module.exports = {
  getAllCastecertificates: async (req, res) => {
    try {
      ...
      const allCastecertificate = await Castecertificate.find({
        school: schoolId,
      })
        .populate("class")
        .populate("section")
  ...
  createCastecertificate: (req, res) => {
    const schoolId = req.user.schoolId;
    const newCastecertificate = new Castecertificate({
      ...req.body,
      school: schoolId,
    });
    newCastecertificate
  ...
  getCastecertificateWithId: async (req, res) => {
    ...
    const schoolId = req.user.schoolId;
    Castecertificate.findOne({ _id: id, school: schoolId })
      .populate("class")
      .populate("section")
      .populate("student")
      .populate("castecategory")
  ...
  updateCastecertificateWithId: async (req, res) => {
    ...
    try {
      ...
      await Castecertificate.findOneAndUpdate(
        { _id: id },
        { $set: { ...req.body } },
      );
      const CastecertificateAfterUpdate = await Castecertificate.findOne({
        _id: id,
      })
        .populate("class")
        .populate("section")
  ...
  deleteCastecertificateWithId: async (req, res) => {
    try {
      ...
      const CastecertificateAfterDelete = await Castecertificate.findOne({
        _id: id,
      })
        .populate("class")
        .populate("section")
