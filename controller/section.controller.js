...
module.exports = {
  getAllSections: async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message: "Server Error in Getting All Section. Try later",
      });
    }
  },
  ...
  getSectionWithId: async (req, res) => {
    ...
    Section.findOne({ _id: id, school: schoolId })
      ...
      .then((resp) => {
        if (resp) {
          ...
          res
            .status(500)
            .json({ success: false, message: "Section data not Available" });
        }
      })
      .catch((e) => {
        console.log("Error in getSectionWithId", e);
        res
          .status(500)
          .json({ success: false, message: "Error in getting  Section Data" });
      });
  },
  ...
  updateSectionWithId: async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({
        ...
        data: SectionAfterUpdate,
      });
    } catch (error) {
      console.log("Error in updateSectionWithId", error);
      res.status(500).json({
        success: false,
        message: "Server Error in Update Section. Try later",
      });
    }
  },
  deleteSectionWithId: async (req, res) => {
    try {
      const schoolId = req.user.schoolId;
      let id = req.params.id;
      const subExamCount = (await Exam.find({ section: id, school: schoolId }))
        .length;
      const subPeriodCount = (
        await Period.find({ section: id, school: schoolId })
      ).length;
      ...
      res.status(500).json({
        success: false,
        message: "Server Error in Deleting Section. Try later",
      });
    }
  },
  getAttendeeTeacher: async (req, res) => {
    try {
      ...
      let attendeeSection = await Attendee.find({
        teacher: req.user.id,
      }).populate("section");

      attendeeSection = attendeeSection.map((x) => {
        return {
          section_code: x?.section?.section_code,
          section_name: x?.section?.section_name,
          sectionId: x?.section?._id,
        };
      });
      res.status(200).json(attendeeSection);
    } catch (error) {
      ...
      res.status(500).json({
        success: false,
        message: "Server Error in getting  Attendee. Try later",
      });
    }
  },
};
