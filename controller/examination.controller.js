...
module.exports = {
    ...
    getExaminationByClass: async(req, res)=>{
        try {
            ...
            const examination = await Examination.find({class:req.params.classId,school:schoolId}).populate("subject").populate("examtype");
    ...
    getAllExaminations:async(req, res)=>{
        try {
            const examinations = await Examination.find().populate("subject").populate("class").populate("examtype");
